...
#    Updated: 2021/09/24 12:31:24 by ablaamim         ###   ########.fr        #
#                                                                              #
# **************************************************************************** #
...
function	check_c00_ex07()
{
	...
	cd $usr_out
	local NUMBER=(42 2147483647 1337 -42 -2147483647 -1337 6473287 -328 612567 9)
	local i=0
	while [ $i -le 8 ]
	do
		local USER_OUTPUT=$(./user.out ${NUMBER[$i]})
...
function	check_c00_ex08()
{
	...
	local RES00="012345678, 012345679, 012345689, 012345789, 012346789, 012356789, 012456789, 013456789, 023456789, 123456789"
	printf "> test 1\n" >> $current_dir/DEEPTHOUGHT
	cd $usr_out
	local USER_OUTPUT=$(./user.out)
